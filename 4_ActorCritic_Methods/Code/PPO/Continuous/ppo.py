...
class PPO_agent:
    def __init__(self, state_dim, action_dim, gamma=0.99, lambd=1.0, a_lr=1e-3, c_lr=1e-3, eps_clip=0.1, v_loss_coef=0.5, e_loss_coef=0.001) -> None:
        ...
        self.optimizer = torch.optim.Adam([
            {'params': self.ppo_net.mean_net.parameters(), 'lr': a_lr},
            {'params': self.ppo_net.std_net.parameters(), 'lr': a_lr},
    ...
    def get_value(self, state):
        with torch.no_grad():
            _, _, value = self.ppo_net(torch.tensor(state).unsqueeze(0).float().cuda())
        return value

    def get_action(self, state, choose_max = False):
        with torch.no_grad():
            mu, sigma, _ = self.ppo_net(torch.tensor(state).unsqueeze(0).float().cuda())
            # sigma to covariance if act_dim > 1
            cov_matrix = torch.diag_embed(sigma)
            m = MultivariateNormal(mu, cov_matrix)
            action = m.sample()
    ...
    def bootstrap_traj(self):
        ...
        for t in reversed(range(traj_len)):
            ...
            with torch.no_grad():
                _, _, value = self.ppo_net(state)
                _, _, value_ = self.ppo_net(state_)
